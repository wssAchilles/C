...
int **flagged;  // 存储格子是否被标记为旗帜（0为未标记，1为标记）
...
void flag_cell(int x, int y) {
    if (revealed[x][y] == 0) {
        flagged[x][y] = !flagged[x][y];  // 切换旗帜标记状态
    }
}
...
int check_win() {
    ...
    if (revealed_count == SIZE * SIZE - MINES && flagged_count == MINES) {
        return 1;  // 胜利
    }
    return 0;  // 继续游戏
}
...
int main() {
    ...
    while (!game_over) {
        ...
        printf("请输入操作：\n");
        printf(" 1. 点击格子 (输入: k 行 列)\n");
        printf(" 2. 标记旗帜 (输入: f 行 列)\n");
        printf(" 3. 退出游戏 (输入: q)\n");
